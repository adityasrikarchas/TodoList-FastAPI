...
class TodoRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            'example': {
                'title': 'Harry Potter',
                'description': 'Hogwarts Fantasy',
                'priority': 5,
                'complete': False
            }
        }

@router.get('/', status_code=status.HTTP_200_OK, summary='Read All Todos')
...
async def read_todo(user: user_dependancy, db: db_dependancy,
...
async def update_todo(user: user_dependancy, db: db_dependancy,
                      todo_id: int = Path(gt=0), todo_request: TodoRequest = Body()):
    if user is None:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Authentication Failed')
